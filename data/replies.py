...
class TaskReply(SqlAlchemyBase, SerializerMixin):
    ...
    seeker_id = sqlalchemy.Column(sqlalchemy.Integer, sqlalchemy.ForeignKey('seekers.id'))
...
def add_job_reply(job:Job, content:str, sender:Seeker) -> JobReply:
    session = create_session()
    new_job_reply = JobReply()
    new_job_reply.job = job
    new_job_reply.content = content
    new_job_reply.seeker = sender
